...
import GenderSelect from '@/components/molecules/GenderSelect.vue'
...
async function openModal() {
    ...
    const params = {
        ...
        type: props.meta.utm_source == 'chatbot_ro' ? 'ro' : 'new_customer',
...
const onGenderChange = async (genderNew) => {
    store.commit('isLoading', true);
...
const onSubmit = async () => {
    ...
    await store.dispatch('updateFollowup', params)
...
<template>
    <button type="button" @click="openModal" :class="props.color"
        class="w-[24px] h-[24px] rounded-full inline-flex items-center justify-center p-[6px] text-white"
        :disabled="props.color == 'bg-[#CCCCCC]'">
        <slot />
    </button>
                            ...
                            <form @submit.prevent="onSubmit">
                                <div class="flex flex-col">
                                    <div class="inline-flex items-center">
                                        <span class="text-[12px] mr-[14px] text-[#227900] font-bold">Jenis
                                            Kelamin</span>
                                        <GenderSelect
                                            :genderActive="props.form?.meta?.find(v => v.label == 'Jenis Kelamin')?.value"
                                            @response="onGenderChange" />
                                    </div>
